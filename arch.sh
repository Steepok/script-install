...
###############################################################################
DEVICE="/dev/nvme0n1"          # Твой диск (у меня NVMe). ЗАМЕНИТЕ ЕСЛИ НУЖНО.
EFI_SIZE="+512M"
SWAP_SIZE="+5G"
...
###############################################################################
if [ "$(id -u)" -ne 0 ]; then
  echo "Запусти этот скрипт с правами root (sudo)."
  exit 1
fi

read -r -p "Внимание: диск ${DEVICE} будет полностью удалён. Продолжить? (yes/NO) " CONF
if [ "$CONF" != "yes" ]; then
  echo "Отмена."
  exit 1
fi
